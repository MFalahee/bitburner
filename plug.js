...
export async function main(ns) {
	...
	await ns.write(files[0], 'grow(args)', 'w'); await ns.write(files[1], 'weaken(args)', 'w'); await ns.write(files[2], 'hack(args)', 'w');

	var exclude = ['home'] //Servers names that won't be used as hosts or deleted

	var servers; var hosts; var targets; var exes; var tarIndex; var loop; var hType;
	var netManager = false; var serverManager = false; var tmp; var act;
	var cycle = [0, '▄', '█', '▀', '█'];
	...
	async function hackAll() {//Dedicates high RAM servers to high value ones
		for (let host of hosts) {
			...
			function fRam() {
				...
				if (host[1] == 'home') { return Math.max(f - 50, 0) } else { return f }
			}
			if (info('MA', target) < info('MM', target) * .80) { hType = 0 }
			else if (info('SL', target) > info('MSL', target) + 5 || loop) {
				if (fRam() / info('MR', host[1]) > .13 && fRam() > 4) {
					hType = 1; tmp = Math.floor(fRam() / 1.75); if (tmp > 0) { ns.exec(files[1], host[1], tmp, target) }
				}
			} else {
				...
				if (hType == 2 && !ns.scriptRunning(files[2], host[1])) {
					if (fRam() < 20) { ns.scriptKill(files[0], host[1]); ns.scriptKill(files[1], host[1]) }
					tmp = Math.floor(fRam() / 1.7); while (parseFloat(ns.hackAnalyze(target)) * tmp >= .7) { tmp-- }
					if (tmp > 0) { ns.exec(files[2], host[1], tmp, target) }
				}
			}
			if ((hType == 0 || hType == 2) && fRam() / info('MR', host[1]) > .13 && fRam() > 4) {
				tmp = [Math.ceil(fRam() / 1.75 * .15), Math.floor(fRam() / 1.75 * .80)]
				if (tmp[0] > 0) { ns.exec(files[1], host[1], tmp[0], target) }
				if (tmp[1] > 0) { ns.exec(files[0], host[1], tmp[1], target) }
			}
	...
	while (true) {//Keeps everything running once per second 
		servers = []; targets = []; hosts = [[info('MR', 'home'), 'home']]; exes = []
